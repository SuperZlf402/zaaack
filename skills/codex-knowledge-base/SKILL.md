...
7. Always use a stable device-specific folder name under GitHub, based on the real computer name from `sources.json` such as `SUPERZLFSURFACE`. Do not use generic names such as `current-computer`, because they will overwrite other devices.
...
When uploading the export, read `sources.json` and use its `computer_slug` value as the target folder:

```text
codex/computers/<computer_slug>/
```

If `computer_slug` is missing because the export was generated by an older script, derive it from `computer_name` by replacing unsafe path characters and spaces with hyphens. Never upload a new device into `codex/computers/current-computer/`.
...
- Before copying files into the repo, inspect `sources.json` and set the destination to `codex/computers/<computer_slug>/`; do not reuse `current-computer`.
